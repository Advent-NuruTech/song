...
drop function if exists public.get_media_feed(text, boolean, integer, timestamptz, uuid, integer);

create function public.get_media_feed(
  p_media_type text,
...
returns setof public.media language sql stable security definer set search_path=public as $$
  ...
  where m.media_type = p_media_type
    and m.is_published
    and not m.deleted
    and (nullif(btrim(p_category), '') is null or lower(m.category) = lower(btrim(p_category)))
...
create index if not exists idx_media_published_category_feed
  on public.media (media_type, lower(category), is_featured desc, sort_order asc, published_at desc, id desc)
  where is_published and not deleted;

revoke all on function public.get_media_feed(text, boolean, integer, timestamptz, uuid, integer, text) from public;
revoke all on function public.search_media(text, text, integer, text) from public;
grant execute on function public.get_media_feed(text, boolean, integer, timestamptz, uuid, integer, text) to anon, authenticated;
grant execute on function public.search_media(text, text, integer, text) to anon, authenticated;
