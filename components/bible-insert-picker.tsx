...
export function BibleInsertPicker({ visible, onClose, onInsert }: {
  ...
  const openChapter = useCallback(async (nextBook: BibleBook, nextChapter: number, range?: { start?: number; end?: number; all?: boolean }) => {
    ...
    try {
      ...
      const selected = range?.all
        ...
        : [];
      setSelectedVerses(new Set(selected));
  ...
  const selectedRows = useMemo(() => {
    ...
    return verses.filter((item) => selectedVerses.has(item.verse));
  ...
  const insert = useCallback(() => {
    ...
    const formatted = formatScriptureSelection({
      ...
      entireChapter: allChapterSelected,
    });
    if (!formatted) return;
    const accepted = onInsert({ ...formatted, versionId: version.id, versionLabel });
    if (accepted !== false) onClose();
  }, [allChapterSelected, book, chapter, onClose, onInsert, previewSelection, selectedRows, version]);

  const toggleVerse = (verse: number) => setSelectedVerses((current) => {
...
const styles = StyleSheet.create({
  container: { flex: 1 }, header: { minHeight: 72, borderBottomWidth: 1, paddingHorizontal: 18, flexDirection: "row", alignItems: "center" }, headerCopy: { flex: 1 }, title: { fontSize: 20, fontWeight: "900" }, subtitle: { fontSize: 11, marginTop: 3 }, iconButton: { width: 42, height: 42, alignItems: "center", justifyContent: "center" }, center: { flex: 1, padding: 28, alignItems: "center", justifyContent: "center", gap: 12 },
  ...
  previewPanel: { position: "absolute", left: 10, right: 10, bottom: 10, borderWidth: 1, borderRadius: 18, padding: 12, shadowColor: "#000", shadowOffset: { width: 0, height: 4 }, shadowOpacity: .18, shadowRadius: 12, elevation: 8 }, previewHeader: { flexDirection: "row", justifyContent: "space-between", marginBottom: 8 }, previewTitle: { fontSize: 14, fontWeight: "900" }, previewHelp: { fontSize: 10, marginTop: 2 }, previewRef: { fontSize: 11, fontWeight: "900" }, previewInput: { minHeight: 62, maxHeight: 105, borderWidth: 1, borderRadius: 11, padding: 10, fontSize: 14, lineHeight: 20, textAlignVertical: "top" }, insertButton: { minHeight: 44, marginTop: 9, borderRadius: 12, flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 8 },
});
