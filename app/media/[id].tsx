...
import { formatMediaCount, mediaDescriptionToPlainText } from "@/src/features/media/utils";
...
export default function MediaDetailScreen() {
  ...
  const plainDescription = mediaDescriptionToPlainText(item.description);
  const richDescription = isRichStudyHtml(item.description);
    ...
    <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false} stickyHeaderIndices={[0]}>
      ...
      <View style={styles.copy}>
        ...
        <View style={[styles.descriptionCard, { backgroundColor: colors.card, borderColor: colors.border }]}>
          ...
          {richDescription
            ? <View style={styles.richDescription}><StudyRichContent html={item.description} colors={colors} size={size} fontFamily={fontFamily} /></View>
            : <Text numberOfLines={descriptionExpanded ? undefined : 4} style={[styles.description, { color: colors.text, fontFamily }]}>{plainDescription || "No description provided."}</Text>}
          {!richDescription && plainDescription.length > 180 ? <Pressable accessibilityRole="button" onPress={() => setDescriptionExpanded((value) => !value)} hitSlop={8}><Text style={[styles.readMore, { color: colors.tint, fontFamily }]}>{descriptionExpanded ? "Show less" : "Read more"}</Text></Pressable> : null}
        </View>
...
const styles = StyleSheet.create({
  screen: { flex: 1 }, content: { paddingBottom: 100 }, playerWrap: { backgroundColor: "#000", zIndex: 10, elevation: 10 }, floatingBack: { position: "absolute", top: 12, left: 12, width: 40, height: 40, borderRadius: 20, alignItems: "center", justifyContent: "center", backgroundColor: "rgba(0,0,0,.6)" }, copy: { padding: 18 }, title: { fontWeight: "900", lineHeight: 29 }, meta: { fontSize: 12, marginTop: 8 }, actions: { borderTopWidth: 1, borderBottomWidth: 1, marginTop: 17, paddingVertical: 5, alignItems: "flex-start" }, descriptionCard: { marginTop: 18, borderRadius: 16, borderWidth: 1, padding: 15 }, sectionTitle: { fontSize: 15, fontWeight: "900" }, description: { fontSize: 14, lineHeight: 22, marginTop: 8 }, richDescription: { marginTop: 12 }, readMore: { fontSize: 13, fontWeight: "900", marginTop: 8 }, commentsButton: { minHeight: 70, borderBottomWidth: 1, flexDirection: "row", alignItems: "center", justifyContent: "space-between", marginTop: 8 }, commentCount: { fontSize: 12, marginTop: 3 }, center: { flex: 1, alignItems: "center", justifyContent: "center", padding: 30 }, errorTitle: { fontSize: 20, fontWeight: "900", marginTop: 12 }, errorBody: { fontSize: 13, textAlign: "center", marginTop: 5 }, backButton: { borderRadius: 12, marginTop: 18, paddingHorizontal: 20, paddingVertical: 12 }, backText: { color: "#fff", fontWeight: "800" },
